...
This is what `adk deploy cloud_run --with_ui ./examples/agents/fleet` ships, and what
...
it, so the fleet is built at import time.
    ...
    adk deploy cloud_run --project=$GOOGLE_CLOUD_PROJECT \
        --region=us-central1 --with_ui ./examples/agents/fleet
...
from adk_harness.coding.adapters import ClaudeCodeHarness, CodexHarness
from adk_harness.coding.fleet import build_fleet_sync
...
class WorkspacePolicy:
    ...
    async def check(self, request: PolicyRequest) -> Decision:
        ...
        candidate = Path(resource).resolve()
        if root not in (candidate, *candidate.parents):
            return Decision(
                outcome=DecisionOutcome.deny,
                reason=(
                    f"{candidate} is outside the workspace {root}. "
