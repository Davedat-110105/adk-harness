...
This is the one that is not a demo. `examples/fleet` registers a stub so the
...
work under the governance gate.
    ...
    python examples/dogfood.py "Add a docstring to src/adk_harness/registry.py"
    python examples/dogfood.py --cwd ../coactra "Find unused exports"
    python examples/dogfood.py --precedents ~/.adk-harness.db "..."
...
not individually gated. Read `src/adk_harness/agent.py` before pointing this at
