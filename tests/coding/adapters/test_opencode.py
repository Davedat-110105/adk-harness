...
@pytest.mark.asyncio
async def test_concurrent_runs_use_independent_clients(monkeypatch: pytest.MonkeyPatch) -> None:
    _fake_httpx(monkeypatch)
    _AsyncClient.stream_response = _StreamResponse(
        [_sse({"type": "session.idle", "properties": {}}), ""]
    )
    harness = OpenCodeHarness()

    async def collect(prompt: str) -> list[Any]:
        return [turn async for turn in harness.run(prompt, cwd="/repo")]

    await asyncio.gather(collect("one"), collect("two"))

    assert len(_AsyncClient.instances) == 2
    assert _AsyncClient.instances[0] is not _AsyncClient.instances[1]
    assert all(client.closed for client in _AsyncClient.instances)
    assert harness._active_runs == []


@pytest.mark.asyncio
async def test_run_without_httpx_does_not_leave_active_holder(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setitem(sys.modules, "httpx", None)
    harness = OpenCodeHarness()

    turns = [turn async for turn in harness.run("x", cwd="/repo")]
