...
from adk_harness.coding.protocol import HarnessTurn
...
def _install(monkeypatch: pytest.MonkeyPatch) -> _FakeAntigravitySdk:
    ...
    monkeypatch.setattr(
        "adk_harness.coding.adapters.antigravity._localharness_binary", lambda: _BINARY
    )
    return fake
...
async def test_discover_reports_unavailable_when_runtime_binary_missing(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "adk_harness.coding.adapters.antigravity._localharness_binary", lambda: None
    )
