...
@pytest.mark.asyncio
async def test_concurrent_runs_keep_independent_processes(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    pending = [
        FakeProcess(stdout_lines=[_line({"type": "item.completed", "item": {"type": "agent_message", "text": label}})])
    ...
    harness = CodexHarness()
    first, second = harness.run("a", cwd="/tmp"), harness.run("b", cwd="/tmp")

    assert (await first.__anext__()).text == "one"
    assert (await second.__anext__()).text == "two"
    assert len(harness._active_processes) == 2
    await harness.aclose()
    assert all(process.terminated for process in spawned)


@pytest.mark.asyncio
async def test_stdin_failure_still_terminates_process(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    class BrokenStdin(_FakeStdin):
        async def drain(self) -> None:
    ...
    monkeypatch.setattr(asyncio, "create_subprocess_exec", spawn)
    agen = CodexHarness().run("x", cwd="/tmp")
