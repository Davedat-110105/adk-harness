...
def test_record_stores_hash_without_persisting_raw_arguments_by_default() -> None:
    client = FakeClient()
...
def test_explicit_allowlist_persists_only_named_fields() -> None:
    client = FakeClient()
    ledger = FirestoreActionLedger(
        client,
        denylist=("internal",),
        arg_allowlist={"tool.call": ("visible", "internal_note")},
    )

    entry_id = _record(
        ledger,
        "key-custom",
        tool_args={"internal_note": "hidden", "visible": "safe"},
    )

    assert client.documents[entry_id]["tool_args_redacted"] == {"visible": "safe"}


def test_default_metadata_only_mode_drops_secrets_in_lists_and_nested_strings() -> None:
    client = FakeClient()
    ledger = FirestoreActionLedger(client)
    args = {
        "prompt": ["Authorization: bearer definitely-secret", {"body": "password=secret"}],
        "headers": {"x-custom": "token-value"},
    }

    entry_id = _record(ledger, "nested-secret", tool_args=args)

    stored = client.documents[entry_id]
    assert stored["tool_args_redacted"] == {}
    stored_repr = repr(stored)
    assert all(
        secret not in stored_repr
        for secret in ("definitely-secret", "password=secret", "token-value")
    )
